...
{
	if (camera->position.y <= 10 && camera->position.x <= 10 && camera->position.y >= -10 && camera->position.x >= -10)
	{
		return true;
	}
	else
...
void update_camera_position(struct Camera *camera, double elapsed_time)
{
	...
	if (check_collision(camera, -1.0, 1.0, 1.0) == 1 && toggleAttack == true)
	{
		GameEnd = 1;
	}
}

int check_collision(struct Camera *camera, double x, double y, double size)
{

	if ((double)camera->position.x >= x - size && (double)camera->position.x <= x + size && (double)camera->position.y >= y - size && (double)camera->position.y <= y + size)
	{
		return 1;
	}
	else
